...
function initDatabase() {
  ...
  // Миграция: обновляем таблицу maintenance_logs для новых полей
  try {
    const tableExists = db.prepare("SELECT name FROM sqlite_master WHERE type='table' AND name='maintenance_logs'").get();
    
    if (tableExists) {
      const columns = db.prepare("PRAGMA table_info(maintenance_logs)").all();
      
      if (!columns.some(col => col.name === 'maintenance_type_id')) {
        db.exec('ALTER TABLE maintenance_logs ADD COLUMN maintenance_type_id TEXT');
      }
      if (!columns.some(col => col.name === 'next_maintenance_date')) {
        db.exec('ALTER TABLE maintenance_logs ADD COLUMN next_maintenance_date TEXT');
      }
      if (!columns.some(col => col.name === 'notes')) {
        db.exec('ALTER TABLE maintenance_logs ADD COLUMN notes TEXT DEFAULT ""');
      }
      if (!columns.some(col => col.name === 'changed_by')) {

...
router.post('/', writeLimiter, validate(userSchema), (req, res) => {
  const { first_name, last_name, middle_name, email, subdivision_id, position } = req.body;
  const id = uuidv4();
  db.prepare('INSERT INTO users (id, first_name, last_name, middle_name, email, subdivision_id, position) VALUES (?, ?, ?, ?, ?, ?, ?)').run(id, first_name, last_name, middle_name || '', email || '', subdivision_id || null, position || '');
...
router.put('/:id', writeLimiter, validate(userSchema), (req, res) => {
  ...
  db.prepare('UPDATE users SET first_name = ?, last_name = ?, middle_name = ?, email = ?, subdivision_id = ?, position = ? WHERE id = ?').run(
    first_name || existing.first_name,
    last_name || existing.last_name,
    middle_name !== undefined ? middle_name : existing.middle_name,
